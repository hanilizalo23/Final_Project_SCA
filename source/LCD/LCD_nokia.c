...
void DSPI_MasterUserCallback(SPI_Type *base, dspi_master_handle_t *handle, status_t status, void *userData)
{
	...
	xSemaphoreGiveFromISR(Semaphore_SPI,&xHigherPriorityTaskWoken);
...
void LCD_nokia_init(void)
{
	...
	PORT_SetPinMux(PORTB, RESET_PIN, kPORT_MuxAsGpio);
	
	GPIO_PinInit(GPIOD, DATA_OR_CMD_PIN, &led_config);
	GPIO_PinInit(GPIOB, RESET_PIN, &led_config);
	
	GPIO_PortClear(GPIOB, 1u << RESET_PIN);
	LCD_nokia_delay();
	GPIO_PortSet(GPIOB, 1u << RESET_PIN);
...
void LCD_nokia_bitmap(const uint8_t bitmap[]){
	...
	DSPI_MasterTransferNonBlocking(SPI0, &g_m_handle_SPI, &masterXfer);
